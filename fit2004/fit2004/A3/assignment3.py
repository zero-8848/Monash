        ...
        for city_from in range(len(travel_days)):#num of cities
            ...
            if day>0 :#should start from start
                for city_to in range(len(travel_days)):#num of cities
                    ...
                    if (memo[day][city_from]==-1 or travel_days[city_from][city_to]>day or travel_days[city_from][city_to]<=0) and memo[day][city_to]<=0 :
                        memo[day][city_to]=-1
                    #memo[day][city_from]==-1  and 
                    # if revenue[day][city_to]==0 and memo[day][city_to]<=0 and memo[day][city_to]<=0:

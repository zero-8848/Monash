...
def best_revenue(revenue, travel_days,start):
    ...
    #can trvel ,can stay , not accessible 
    for day in range(len(revenue)):#num of days，d

        for city_from in range(len(travel_days)):#city from
            #day0
            if day==0 and city_from==start:
                memo[0][city_from]=revenue[day][start]
                memo_max[day]=revenue[day][start]

            elif day>0 :#can travel from day 1
                for city_to in range(len(travel_days)):#cities to 


                    # can stay
                    if not(day==1 and city_from!=start) and memo[day][city_from]>0:
                    ...
                    #can travel cases
                    if not(memo[day-travel_days[city_from][city_to]-1][city_from]==-1 or travel_days[city_from][city_to]<=0 or day<=travel_days[city_from][city_to]):
                        if day==travel_days[city_from][city_to] and revenue[day][city_to] >  memo[day][city_to]:
                            ...
                            if memo[day][city_from]==-1 and memo[day][city_to]<=0 :
                                memo[day][city_to]=-1
                    elif memo[day-travel_days[city_from][city_to]-1][city_from]==-1 or travel_days[city_from][city_to]<=0 or day<=travel_days[city_from][city_to] and memo[day][city_to]<=0:
                        memo[day][city_to]=-1
